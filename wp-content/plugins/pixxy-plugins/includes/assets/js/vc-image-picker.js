...
if (function($) {
    var pixxyResponsiveAlignmentParam = Backbone.View.extend({
        events: {},
    ...
    // VC pixxy responsive aligment js
    vc.atts.responsive_alignment = {
        parse: function(param) {
            return this.content().find("input.wpb_vc_param_value." + param.param_name).data("pixxyResponsiveAlignment").save().join(" ")
        },
        init: function(param, $field) {
            $('[data-column-offset="true"]', $field).each(function() {
                var $this = $(this);
                $this.find(".wpb_vc_param_value").data("pixxyResponsiveAlignment", new pixxyResponsiveAlignmentParam({
                    el: $this
                }).render())
            })
        }
...
(function($) {
    ...
    vc.atts.gradient = {
        ...
        init: function(param, $field) {
            ...
            var defaults = {
                ...
                targetInputElement: $field.find('.pixxy-gradient-css'),
                targetBgInputElement: $field.find('.pixxy-gradient-bg'),
                startingGradient: startingGradient,
                startingBgProps: startingBgProps
            };

            var options = $.extend(true, {}, defaults, param.settings);
            $field.find('.pixxy-gradient').icsge(options);
        },

        parse: function(param) {
            var $field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]"),
                $css = $field.next('.pixxy-gradient-css'),
                $bg = $css.next('.pixxy-gradient-bg');

            return $css.val() + '|' + $bg.val();
        }
    };

    // VC pixxy slider parameter js
    vc.atts.pixxy_slider = {
        ...
        init: function( param, $field ) {
            ...
            var $handle = $field.find('.pixxy-handle');

            $field.find('.pixxy-slider').slider({
                min: param.min,
                max: param.max,
                step: param.step,
                value: $init_val,
    ...
    vc.atts.pixxy_shape_divider = {
        ...
        init: function(param, $field) {
            $(".pixxy_color-control", $field).each(function() {
            ...
            $( ".pixxy-slider", $field).each( function() {
                var $init_val, $control = $( this ),
                    value  = $control.parent().find( ".pixxy-sliderinput").val(),
                    $handle = $control.find('.pixxy-handle');
                ...
                $control.slider({
                    ...
                    change: function( event, ui ) {
                        $control.parent().find( ".pixxy-sliderinput").val( ui.value );
                    }
                });
            });

            $('h3.pixxy-shape-divider-heading', $field).click(function(e) {
                $(this).next('.pixxy-inner-wrap').removeClass('hidden')
                    .parent().siblings().children('.pixxy-inner-wrap').addClass('hidden');
            });

            $(document).on('change', '.pixxy_shape_divider-type', function() {
    ...
    vc.atts.responsive_css_editor = {
        ...
        init: function(param, $field) {
            $('h3.pixxy-responsive-css-heading', $field).click(function(e) {
                $(this).parent('.pixxy-main-responsive-wrapper').addClass('active').siblings().removeClass('active');
            });
        },

    };

    // VC Advanced pixxy Checkbox js
    vc.atts.pixxy_advanced_checkbox = {

        parse: function(param) {

            if ($(".pixxy-advanced-checkbox input").is(':checked')) {
                var $checked = 'checked="checked"';
            } else {
                var $checked = '';
            }

            var $pixxy_panel = $('#vc_ui-panel-edit-element'),
                $easy_mode_checkbox = $('<label class="pixxy-easy-mode-switch"><input type="checkbox" class="pixxy-easy-mode-checkbox" ' + $checked + ' value="true">Easy Mode<span><i class="fa fa-power-off"> </i></span></label>'),
                $easy_mode;

            $pixxy_panel.find('.pixxy-easy-mode-switch').remove();
            $pixxy_panel.find('.vc_ui-panel-header-controls').prepend($easy_mode_checkbox);
            $easy_mode = $easy_mode_checkbox.find('.pixxy-easy-mode-checkbox');

            var $advanced_checkbox = $(".pixxy-advanced-checkbox input");
    ...
    // VC pixxy Checkbox js
    vc.atts.pixxy_checkbox = {
    ...
    function templates_pixxy() {

        if(jQuery('.pixxy-templates-container').length){
            jQuery('.pixxy-templates-container > .vc_ui-list-bar').each(function () {
                var layoutM = 'masonry';
                jQuery(this).isotope({
    ...
    vc.TemplateWindowUIPanelBackendEditor = vc.TemplatesPanelViewBackend.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperTemplatesPanelViewSearch).extend({
        ...
        show: function() {
            ...
            this.clearSearch(), vc.TemplateWindowUIPanelBackendEditor.__super__.show.call(this), $('.vc_edit-form-tab[data-tab="pixxy_templates"] .sortable_templates ul > li').each(function() {
                "all" == $(this).attr("data-sort") ? $(this).find(".count").html($('.vc_edit-form-tab[data-tab="pixxy_templates"] .vc_ui-template-list > .vc_ui-template').length/2) : $(this).find(".count").html($('.vc_edit-form-tab[data-tab="pixxy_templates"] .vc_ui-template-list > .vc_ui-template.' + $(this).attr("data-sort")).length/2);
            ...
            //
            // jQuery('.pixxy-templates-container img').on('load', function () {
            //
            //     alert('hi');
