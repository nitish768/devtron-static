...
var EnvatoWizard = (function($){
    ...
    function stepsload() {
        var directory = $('.envato-setup-content h2[data-directory]').eq(0).attr('data-directory');

        $('.envato-setup-content .envato-setup-actions.step.hide').hide();
        $('.envato-setup-content h2[data-directory]').hide();
        $('.envato-setup-content h2[data-directory]').eq(0).show().addClass('active');
        $('.envato_default_content[data-directory]').hide();
        $('.envato_default_content[data-directory="'+ directory +'"]').show();
        $('.custom-steps a.prev').hide();
    }

    $('.all-pages .item.lab').on('click', function () {
        $('.envato_default_content input').prop('checked', true);
        $('.envato_default_content').addClass('checkes');

        var directory = $('.envato-setup-content h2[data-directory]').last().attr('data-directory');
        $('.envato-setup-content h2[data-directory]').last().show().addClass('active');
        $('.envato_default_content[data-directory="'+ directory +'"]').show();
        $('.envato-setup-actions .hide').show();
        $('.all-pages').hide();

    });

    $('.all-pages .item:not(.lab)').on('click', function () {
        stepsload();
        $(this).addClass('checkes');
        $('.custom-steps').show();
        $('.envato-setup-actions .hide').show();
        $('.all-pages').hide();
    });
    ...
    $(document).on('ready', function () {
        ...
        $('.custom-steps').hide();
        $('.envato-setup-actions .hide').hide();

    });

    $(window).on('load', function () {
        $('.importer-pages-wrap').css('opacity', '1');
    });
    ...
    $('.custom-steps a.next').on('click', function () {
       var dir = $('.envato-setup-content h2.active[data-directory]').next().attr('data-directory');
       $('.envato-setup-content h2.active[data-directory]').removeClass('active').hide().next().addClass('active').show();
        $('.envato_default_content[data-directory]').hide();
        $('.envato_default_content[data-directory="'+ dir +'"]').show();
        $('.custom-steps a.prev').show();
        if($('.envato-setup-content h2.active[data-directory]').attr('data-directory') == '4.default'){
            $('.custom-steps').hide();
            $('.envato-setup-content .envato-setup-actions.step.hide').show();
        }else{
            $('.custom-steps').show();
            $('.envato-setup-content .envato-setup-actions.step.hide').hide();
        }
    ...
    $('.custom-steps a.prev:not(.disable)').on('click', function () {
        var dir = $('.envato-setup-content h2.active[data-directory]').prev().attr('data-directory');
        $('.envato-setup-content h2.active[data-directory]').removeClass('active').hide().prev().addClass('active').show();
        $('.envato_default_content[data-directory]').hide();
        $('.envato_default_content[data-directory="'+ dir +'"]').show();
        if($('.envato-setup-content h2.active[data-directory]').attr('data-directory') == '1.homepages'){
            $('.custom-steps a.prev').hide();
        }else{
            $('.custom-steps').show();
            $('.envato-setup-content .envato-setup-actions.step.hide').hide();
    ...
    function ContentManager(){
        ...
        function find_next(){
            ...
            var $enabled_items = $('div.envato_default_content input:checked');
            $items.each(function(){
                if (current_item == '' || do_next) {
