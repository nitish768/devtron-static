...
/**
 * Class Pixxy_Vc_Templates_Editor
 * @since 1.0
 */
class Pixxy_Vc_Templates_Editor {
	/**
	 ...
	 * @var bool
	 */
	protected $pixxy_templates = false;
	/**
	 * @since 4.4
	...
	public function addTemplatesTab( $data ) {
		$newCategory = array(
			'category'        => 'pixxy_templates',
			'category_name'   => esc_html__( 'Pixxy Templates', 'pixxy' ),
			'category_weight' => 1,
	...
	protected function get_template_categories() {
		...
		$categories = array(
			'all'         => esc_html__( 'All', 'pixxy' ),
			'about'      => esc_html__( 'About', 'pixxy' ),
			'banner'      => esc_html__( 'Banners', 'pixxy' ),
			'coming-soon'         => esc_html__( 'Coming soon', 'pixxy' ),
			'contacts'    => esc_html__( 'Contacts', 'pixxy' ),
			'headings'        => esc_html__( 'Headings', 'pixxy' ),
			'instagram'        => esc_html__( 'Instagram', 'pixxy' ),
			'line-of-images'        => esc_html__( 'Line of images', 'pixxy' ),
			'portfolio'        => esc_html__( 'Portfolio', 'pixxy' ),
			'posts'        => esc_html__( 'Posts', 'pixxy' ),
			'services'     => esc_html__( 'Services', 'pixxy' ),
			'skills'        => esc_html__( 'Skills', 'pixxy' ),
			'teams'        => esc_html__( 'Team', 'pixxy' ),
			'pricing'     => esc_html__( 'Pricing', 'pixxy' ),
			'testimonials' => esc_html__( 'Testimonials', 'pixxy' ),
			'accordion' => esc_html__( 'Accordion', 'pixxy' ),
			'call-to-action' => esc_html__( 'Call to action', 'pixxy' ),
			'category' => esc_html__( 'Category', 'pixxy' ),
			'content-block' => esc_html__( 'Content block', 'pixxy' ),
			'counter' => esc_html__( 'Counter', 'pixxy' ),
			'discount' => esc_html__( 'Discount', 'pixxy' ),
			'info-list' => esc_html__( 'Info list', 'pixxy' ),
			'line-of-icons'        => esc_html__( 'Line of icons', 'pixxy' ),
			'products-slider'        => esc_html__( 'Products slider', 'pixxy' ),
			'sliders'        => esc_html__( 'Sliders', 'pixxy' ),
	...
	public function renderTemplateBlock( $category ) {
		...
		if ( 'pixxy_templates' === $category['category'] ) {

			$category['output'] = '<div class="vc_col-md-2 pixxy-sorting-container">';
			$category['output'] .= $this->get_template_categories();
			$category['output'] .= '</div>';


			$category['output'] .= '
			<div class="vc_column vc_col-sm-12 pixxy-templates-container">
	...
	function renderTemplateWindow( $template_name, $template_data ) {
		if ( 'pixxy_templates' === $template_data['type'] ) {
			return $this->renderTemplateWindowPixxyTemplates( $template_name, $template_data );
		}
	 ...
	 * @return string
	 */
	public function renderTemplateWindowPixxyTemplates( $template_name, $template_data ) {
		ob_start();
		...
		$preview_template_title = esc_attr( 'Preview template', 'pixxy' );
		$add_template_title = esc_attr( 'Add template', 'pixxy' );
...
HTML;
	...
	public function renderBackendTemplate() {
		...
		if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
			die( 'Error: Vc_Pixxy_Templates::renderBackendTemplate:1' );
		}
	...
	public function getAllTemplates() {
		...
		$data = array();
		$pixxy_templates = $this->getTemplates();
		$category_templates = array();
		foreach ( $pixxy_templates as $template_id => $template_data ) {
			$category_templates[] = array(
				'unique_id' => $template_id,
				'name' => $template_data['name'],
				'new' => isset( $template_data['new'] ) ? $template_data['new'] : false,
				'type' => 'pixxy_templates',
	...
	public function loadDefaultTemplates() {
		...
		if ( ! is_array( $this->pixxy_templates ) ) {
			$this->pixxy_templates = $this->allTemplates();
		}

		return $this->pixxy_templates;
	}
	...
	public function getDefaultTemplate( $template_index ) {
		...
		if ( ! is_numeric( $template_index ) || ! is_array( $this->pixxy_templates ) || ! isset( $this->pixxy_templates[ $template_index ] ) ) {
			return false;
		}

		return $this->pixxy_templates[ $template_index ];
	}
	...
	public function addDefaultTemplates( $data ) {
		if ( is_array( $data ) && ! empty( $data ) && isset( $data['name'], $data['content'] ) ) {
			if ( ! is_array( $this->pixxy_templates ) ) {
				$this->pixxy_templates = array();
			}
			$this->pixxy_templates[] = $data;
	...
	public function renderTemplateListItem( $template ) {
		$name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'pixxy' ) );
...
HTML;
		$output .= '<div class="pixxy-template-preview">';
		if ( $new ) { $output .= '<span class="pixxy-badge-new">New</span>'; }
		...
		$output .= '<span class="pixxy-template-categories">' . esc_html( $template_sort_name ) . '</span>';
		$output .= <<<HTML
						</div>
